...
async def get_activity_detail(
    ...
    # Build query
    query = '''
        SELECT project_id, user_id, date, commits, issues_created, issues_closed, mrs_created, mrs_merged
        FROM "gitlab-activity-analysis-schema".user_project_activity_cache
        WHERE date >= CURRENT_DATE - INTERVAL '%s days'
    '''
    params = [days]
    if user_id_list:
...
async def get_code_change_detail(
    ...
    # Build query
    query = '''
        SELECT contributor_name, contributor_email, project_id, project_name, additions, deletions, changes, period_start, period_end, created_at
        FROM "gitlab-activity-analysis-schema".contributor_code_churn
        WHERE period_end >= CURRENT_DATE - INTERVAL '%s days'
    '''
    params = [days]
    if user_id_list:
